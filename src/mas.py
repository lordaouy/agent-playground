...
def get_initial_plan(industry, use_case, user_query):
    print("hummm")
    print(industry, use_case, user_query)
    ...
    try:
        ...
        with st.spinner("Processing your query and generating initial agent composition and plan..."):
            completion = client.beta.chat.completions.parse(
                model="GPT4o",  # Replace with your actual model deployment name
            messages=[
                    {"role": "user", "content": user_message}
                ],
                response_format=OverallResponse,
            )
        loading_placeholder.empty()
        #if statement if using a struciured model 
        event = completion.choices[0].message.parsed

        # Debug: Print the entire parsed event
        print("----- Phase 1: Task Decomposition Planner Agent Output -----")
        #print(event)

        # Extract Plan, Short-Term Memory, and Long-Term Memory
        plan = event.Plan
        ...
        #print(plan_json)
        #print(st_memory_json)
        #print(lt_memory_json)
...
def get_initial_plan_message(plan_json, st_memory_json, lt_memory_json):
    ...
    try:
        ...
        st.markdown(event.content, unsafe_allow_html=True)
        st.session_state.plan = plan_json if 'plan_json' in locals() else {}
        st.session_state.st_memory = st_memory_json if 'st_memory_json' in locals() else {}  
        st.session_state.lt_memory = lt_memory_json if 'lt_memory_json' in locals() else {} 
        ...
        print("----- Phase 1: Task Decomposition Planner Agent Output Message -----")
        #print(event.content)
...
def orchestrate_tasks_input(plan_json, st_memory_json, lt_memory_json):
    ...
    try:
        ...
        with st.spinner("Processing your query and generating initial agent composition and plan..."):
            completion = client.beta.chat.completions.parse(
                ...
                response_format=OverallResponse1,  # Adjust if AI Conductor has a different response structure
            )
...
# ------------------- Message output from Phase 1
def orchestrate_tasks_input_message(agent_input_json, plan_json, st_memory_json, lt_memory_json):
    print("Inside 1.22 --------") 
...
def orchestrate_tasks_output(current_task_json, agent_input_json, plan_json, st_memory_json, lt_memory_json):
    ...
    try:
        ...
        with st.spinner("Processing your query and generating initial agent composition and plan..."):
            ...
            response_format=OverallResponse2,  # Adjust if AI Conductor has a different response structure
        )
        loading_placeholder.empty()
...
def orchestrate_tasks_output_message(agent_output_json, plan_json, st_memory_json, lt_memory_json):
    print("TESSSSSSST")
...
print("----- Phase 4: AI Conductor Input 2 -----")
def orchestrate_tasks_input_loop(agent_output_json, plan_json, st_memory_json, lt_memory_json, next_task_json, next_agent_input_json):
...
print("----- Phase 5: AI Conductor Output 2 Loop -----")
def orchestrate_tasks_output_loop(current_task_json, agent_input_json, plan_json, st_memory_json, lt_memory_json):
    if not all([current_task_json, agent_input_json, plan_json, st_memory_json, lt_memory_json]):
        print("Missing required inputs for AI Conductor.")
        return
    ...
    try:
        ...
        #print(st_memory_json)
        #print(lt_memory_json)
        #print(next_task_json)
        #print(next_agent_input_json)
...
def summarize_final_output(plan_json):
